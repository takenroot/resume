...
function exportPdf() { if (typeof pdfMake !== 'undefined' && pdfMake.fonts && pdfMake.fonts.noto) { var name = (cvData && cvData.profile && cvData.profile.name ? cvData.profile.name : '简历') + '.pdf'; try { var doc = buildPdfDef(cvData); pdfMake.createPdf(doc).download(name); return; } catch(e) { console.warn('pdfMake export failed, fallback to print:', e); } } window.print(); }
...
initPdfMakeFont().catch(function (e) { console.warn('pdfmake font init failed:', e); });
...
function exportPdf() { var el = document.getElementById('resumeDocument'); if (!el || typeof html2pdf === 'undefined') return; var pages = document.querySelectorAll('.resume-page'); for (var i = 0; i < pages.length - 1; i++) { pages[i].classList.add('html2pdf__page-break'); } var name = (cvData && cvData.profile && cvData.profile.name ? cvData.profile.name : '简历') + '.pdf'; html2pdf().set({ margin: 8, filename: name, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true, letterRendering: true }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' }, pagebreak: { mode: 'css' } }).from(el).save().then(function () { for (var j = 0; j < pages.length; j++) { pages[j].classList.remove('html2pdf__page-break'); } }); }

/* ===========================================================
   ...
   =========================================================== */
const FIELD_ALIAS = { '姓名': 'name', '岗位': 'title', '性别': 'gender', '年龄': 'age', '电话': 'phone', '邮箱': 'email', '头像': 'avatar', '头像url': 'avatar', '求职状态': 'jobStatus', '工作经验': 'workYears', '所在地': 'nativePlace', '籍贯': 'nativePlace', '学校': 'school', '专业': 'major', '主修课程': 'courses', '公司': 'company', '职位': 'position', '简介': 'summary', '工作描述': 'summary', '技能名': 'name', '详情': 'detail', '项目名': 'name', '项目描述': 'summary', '技术栈': '__tags__', '亮点': '__highlights__', '标题': 'heading', '标签': 'tag' };
...
function parsePipeItems(ct, tp, hd, d) { const its = []; const lns = ct.split('\n').map(function (l) { return l.trim(); }).filter(Boolean); let cur = null; const putPeriod = function (o, s) { const dr = parseDateRange(s); if (dr) { o.startDate = dr.startDate; if (dr.endDate) o.endDate = dr.endDate; } else if (s) o.startDate = s; }; for (let j = 0; j < lns.length; j++) { const ln = lns[j]; if (/^[*\-_]\s[*\-_]\s[*\-_]/.test(ln) || /^-{3,}$/.test(ln)) { if (cur) its.push(cur); cur = null; continue; } const pm = ln.match(/^\*\*(.+?)\*\*\s*\|\s*(.+?)(?:\s*\|\s*(.+?))?(?:\s*\|\s*(.*))?$/); if (pm) { if (cur) its.push(cur); cur = {}; const ps = [pm[1], pm[2], pm[3] || '', pm[4] || '']; if (tp === 'education') { putPeriod(cur, ps[0].trim()); cur.school = ps[1].trim(); cur.major = ps[2].trim(); const l = ps[3].trim(); if (l) cur.courses = l.replace(/.*?主修课程[：:]\s*/, '').trim(); } else if (tp === 'experience' || tp === 'timeline') { putPeriod(cur, ps[0].trim()); if (tp === 'experience') { cur.company = ps[1].trim(); cur.position = ps.slice(2).filter(Boolean).join(' | ').trim(); } else { cur.heading = ps[1].trim(); const tg = ps[2] ? ps[2].trim() : ''; if (tg) cur.tag = tg; } } else if (tp === 'projects') { putPeriod(cur, ps[0].trim()); cur.name = ps[1].trim(); const rl = ps.slice(2).filter(Boolean).join('，').trim(); if (rl) cur.summary = rl; } continue; } if ((ln.startsWith('* ') || ln.startsWith('- ')) && cur) { const t = ln.replace(/^[*-]\s*/, '').replace(/\*\*(.+?)\*\*/g, '$1').trim(); if (t) { if (!cur.highlights) cur.highlights = []; cur.highlights.push(t); } } } if (cur) its.push(cur); if (its.length > 0) d.sections.push({ type: tp, title: hd, items: its }); }

function buildMarkdown(d) {
  const lns = []; const p = d.profile || {};
  lns.push('## 个人信息', ''); if (p.name) lns.push('- **姓名**：' + p.name); if (p.title) lns.push('- **岗位**：' + p.title); if (p.workYears) lns.push('- **工作经验**：' + p.workYears); if (p.nativePlace) lns.push('- **籍贯**：' + p.nativePlace); if (p.jobStatus) lns.push('- **求职状态**：' + codeLabel('jobStatus', p.jobStatus)); if (p.gender || p.birthDate) { const age = p.birthDate ? computeAge(p.birthDate) : ''; lns.push('- **基本信息**：' + (p.gender || '') + (p.gender && age ? ' / ' : '') + age); } if (p.phone) lns.push('- **电话**：' + p.phone); if (p.email) lns.push('- **邮箱**：' + p.email); if (p.github) lns.push('- **GitHub**：' + p.github);

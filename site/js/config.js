...
const SECTION_CONFIG = {
  ...
  projects: { label: '项目经验', fields: [{ n: 'name', l: '项目名' }, { n: 'period', l: '时间' }, { n: 'tags', l: '技术栈 (逗号分隔)', t: 'textarea', a: true }, { n: 'summary', l: '项目描述', t: 'textarea' }, { n: 'highlights', l: '亮点 (每行一条)', t: 'textarea', a: true }, { n: 'challenges', l: '难点 (每行一条)', t: 'textarea', a: true }],
    renderItem: function (i) { const tags = arr(i.tags), th = tags.map(function (t) { return '<li>' + esc(t) + '</li>'; }).join(''); const a = cE('article', 'timeline-item'); let html = '<div class="item-head"><div><h3>' + esc(i.name) + '</h3><ul class="tag-list item-subtitle">' + th + '</ul></div></div>'; if (i.summary) html += '<p class="summary">' + esc(i.summary) + '</p>'; if (i.highlights && i.highlights.length) html += '<div class="item-section"><h4 class="item-section-label">亮点</h4><ul class="item-section-list">' + lis(i.highlights) + '</ul></div>'; if (i.challenges && i.challenges.length) html += '<div class="item-section"><h4 class="item-section-label">难点</h4><ul class="item-section-list">' + lis(i.challenges) + '</ul></div>'; a.innerHTML = html; return a; },

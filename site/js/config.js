...
const SECTION_CONFIG = {
  ...
  projects: { label: '项目经验', fields: [{ n: 'name', l: '项目名' }, { n: 'period', l: '时间' }, { n: 'tags', l: '技术栈 (逗号分隔)', t: 'textarea', a: true }, { n: 'summary', l: '项目描述', t: 'textarea' }, { n: 'highlights', l: '亮点 (每行一条)', t: 'textarea', a: true }, { n: 'challenges', l: '难点 (每行一条)', t: 'textarea', a: true }],
    renderItem: function (i) { const tags = arr(i.tags), th = tags.map(function (t) { return '<li>' + esc(t) + '</li>'; }).join(''); const a = cE('article', 'timeline-item'); a.innerHTML = '<div class="item-head"><div><h3>' + esc(i.name) + '</h3><ul class="tag-list item-subtitle">' + th + '</ul></div></div>' + (i.summary ? '<p class="summary">' + esc(i.summary) + '</p>' : '') + '<ul>' + lis(i.highlights) + '</ul>' + (i.challenges && i.challenges.length ? '<ul>' + lis(i.challenges) + '</ul>' : ''); return a; },
    mdItem: function (i) { const tg = Array.isArray(i.tags) ? i.tags.join('、') : (i.tags || ''); return '**' + (i.period || '') + '** | ' + (i.name || '') + (tg ? ' | ' + tg : '') + '\n' + (i.summary || '') + '\n' + mli(i.highlights) + (i.challenges && i.challenges.length ? '\n' + mli(i.challenges) : ''); },

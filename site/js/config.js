...
const SECTION_CONFIG = {
  ...
  projects: { label: '项目经验', fields: [{ n: 'name', l: '项目名' }, { n: 'role', l: '担任角色' }, { n: 'period', l: '时间' }, { n: 'link', l: '项目链接' }, { n: 'tags', l: '技术栈 (逗号分隔)', t: 'textarea', a: true }, { n: 'summary', l: '项目描述', t: 'textarea' }, { n: 'highlights', l: '亮点 (每行一条)', t: 'textarea', a: true }, { n: 'achievements', l: '项目业绩 (每行一条)', t: 'textarea', a: true }],
    renderItem: function (i) { const tags = arr(i.tags), th = tags.map(function (t) { return '<li>' + esc(t) + '</li>'; }).join(''); const a = cE('article', 'timeline-item'); let html = '<div class="item-head"><div><h3>' + esc(i.name) + (i.role ? ' <span class="item-meta-tag">' + esc(i.role) + '</span>' : '') + '</h3><ul class="tag-list item-subtitle">' + th + '</ul></div><span class="item-time">' + esc(i.period) + '</span></div>'; if (i.summary) html += '<p class="summary">' + esc(i.summary) + '</p>';
      ...
      if (i.highlights && i.highlights.length) html += '<div class="item-section"><h4 class="item-section-label">亮点</h4><ul class="item-section-list">' + lis(i.highlights) + '</ul></div>'; if (i.achievements && i.achievements.length) html += '<div class="item-section"><h4 class="item-section-label">项目业绩</h4><ul class="item-section-list">' + lis(i.achievements) + '</ul></div>'; a.innerHTML = html; return a; },

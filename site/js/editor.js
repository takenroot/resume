...
function buildEditorPrefs() {
  ...
  return '<div class="editor-section editor-section-prefs"><h3>页面设置</h3><div class="prefs-row"><div class="editor-field"><label>主题配色</label><select id="prefTheme">' + to + '</select></div><div class="editor-field"><label>字号</label><select id="prefFontSize">' + so + '</select></div><div class="editor-field"><label>字体</label><select id="prefFontFamily">' + fo + '</select></div></div><div class="prefs-row" style="margin-top:12px"><div class="editor-field"><label>时间轴 · 教育取</label><select id="prefTlEdu">' + ae + '</select></div><div class="editor-field"><label>时间轴 · 工作取</label><select id="prefTlExp">' + ax + '</select></div></div><p class="prefs-hint">打印页边距请在浏览器打印对话框里设置（建议选「无」或「最小」，本项目 @page 已固定 0mm）</p></div>';
}

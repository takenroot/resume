...
const IDENTITY_ICONS = {
  ...
  workYears: 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.2 14.2L11 13V7h1.5v5.2l4.5 2.7-.8 1.3z'
};
...
// 电话保留点击复制 (identity-action 是 app.js 复制委托的钩子类). 意向城市取 expectJobs.cities, 显隐跟 expectJobs 开关走.
...
function renderIdentityEssential(p) {
  ...
  const ej = p.expectJobs || {};
  const cities = Array.isArray(ej.cities) && ej.cities.length ? ej.cities.join('/') : '';
  const items = [['title', p.title], ['phone', p.phone, 'copy'], ['cities', cities, null, 'expectJobs'], ['workYears', p.workYears]];
  let count = 0;
...
// ponytail: 胶囊层 — 必备行之外的可选字段 (籍贯/求职状态/GitHub/微信/邮箱) + 期望标签 (expectJobs 派生).
...
function renderIdentityLine(p) {
  ...
  const items = [['nativePlace', '籍贯'], ['jobStatus', '求职状态'], ['github', 'GitHub', 'link'], ['wechat', '微信'], ['email', '邮箱', 'copy']];
  items.forEach(function (it) {
    const k = it[0], v = p[k];
    if (!v || !String(v).trim() || !isProfileShown(k)) return;
    const dv = k === 'jobStatus' ? codeLabel('jobStatus', v) : String(v); // ponytail: 枚举存码, 显示走 CODE_LABELS.
    let node;
    ...
    const val = it[2] === 'copy' ? '<span>' + esc(dv) + '</span>' : esc(dv); // ponytail: copy 的值包 span, flashCopiedState 需要

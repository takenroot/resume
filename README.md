...
- **头像上传** — 上传后自动转为 base64 存入浏览器 localStorage，按姓名绑定；导出 JSON/Markdown 时不含头像，减轻文件体积。点编辑器里的头像预览可打开裁剪弹窗：拖拽调位置、滑块缩放、自定义选框宽高（证件照 5:7 / 方形 1:1 快捷比例），裁剪参数存 `cv_prefs` 不进数据

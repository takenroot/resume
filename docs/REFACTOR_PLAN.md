...
| **P0** | ⛔ 决策 1 | 隐藏字段 UI（`expectSalary` / `expectCities` / `expectIndustry` / `wechat` / `timeline` 编辑器 + 渲染）+ `renderHeaderExtra` + `.header-extra` —— 数据存但预览永不渲染；**与新增字段决策冲突** | 见下方决策 1                                                                                                                                      | ~80 行（删 UI）/ 或 +30 行（renderer 渲染） | 中                   | **决策待批注**                                             |
| **P0** | 过度设计   | html2canvas + jspdf 两枚 CDN（截图嵌图 PDF）—— 与 `window.print()` 重复功能                                                                                                  | 删 `exportPdfImage` + `captureSequential` + 两个 `<script src=cdn>` 标签 + 编辑器菜单项                                                                 | ~50 行 / 2 文件 / **-2 deps**        | 中（部分用户依赖截图 PDF）     | 这个我俩个都想保留，但是我想重新设计导出菜单，减少歧义。图片就是图片，pdf就是pdf           |
...
| **P1** | 设计缺位   | 编辑器**无 undo/redo** —— 所有操作直接覆盖 cvData + localStorage，无历史栈                                                                                                       | 加最近 N 步快照栈（无需 Command pattern）                                                                                                               | ~60 行 / 2 文件                      | 中（数据结构设计）           | 待批注                                                   |
| **P1** | 设计缺位   | **数据 schema 无运行时校验** —— 导入坏 JSON 让 normalize 抛错，用户看到红字无 UI                                                                                                      | 加 `validateSchema(d)` 走 SECTION_CONFIG 反查，错误 toast 反馈                                                                                        | ~40 行 / 2 文件                      | 中（校验逻辑要覆盖 8 种 type） | 可以，且优先级要高                                             |
| **P1** | 设计缺位   | **导入覆盖无确认** —— `importData` 直接覆盖 cvData + localStorage，旧数据瞬间消失                                                                                                  | 加确认对话框 + 旧版本备份到 `_backup` 槽位                                                                                                                 | ~20 行 / 2 文件                      | 低                   | 可以                                                    |
| **P1** | 设计缺位   | **没有空模板选项** —— 首次打开看到张三示例，手动全删才能开始                                                                                                                              | 加"清空开始 / 加载示例"二选一对话框                                                                                                                         | ~30 行 / 1 文件                      | 低                   | 重置默认按钮当时想到是这个作用，可能还需要再设计                              |
| **P1** | 设计缺位   | **导入后不滚动到顶** —— `importData` 不 `scrollTo(0,0)`                                                                                                                  | 加一行                                                                                                                                          | 1 行                               | 零                   | 可以                                                    |
...
| **P1** | 过度设计   | `prefs.js` THEMES / FONT_SIZES / FONT_FAMILIES 字典（4+3+3=10 选项）—— 单一用户本地简历，预设默认够用                                                                                | 删选择器 UI，留字面量默认（theme/fontSize/fontFamily 直接写死或用最常用项）                                                                                         | ~50 行 / 2 文件                      | 中（用户可能已切换偏好）        | 待批注                                                   |
...
| **P2** | 设计缺位   | **没有键盘快捷键** —— 模块移/添/保存全靠鼠标                                                                                                                                     | 加 `Cmd+Z/Y` `Cmd+↑/↓` `Cmd+S`                                                                                                                | ~40 行 / 2 文件                      | 中（要处理焦点冲突）          | 可以但是优先级靠后                                             |
| **P2** | 设计缺位   | **没有 dark mode** —— `THEMES` 全是浅色，OS dark mode 用户期望落空                                                                                                           | 加 `dark` theme 走 `@media (prefers-color-scheme)`                                                                                             | ~30 行 / 2 文件                      | 低                   | 待批注                                                   |
| **P2** | 设计缺位   | **搜索/定位缺失** —— 内容多了（5+ 段经历）找不到特定条目                                                                                                                              | 编辑器顶部加搜索框，结果高亮                                                                                                                               | ~40 行 / 1 文件                      | 中                   | 待批注                                                   |
| **P2** | 设计缺位   | **打印页边距设计被甩给浏览器** —— `@page { margin: 0 }` 锁死 + 让用户在浏览器对话框改                                                                                                     | 加 `printMargin` pref（none/small/normal）跟 prefs.js 集成                                                                                         | ~30 行 / 2 文件                      | 低                   | 这个之前也让ai改过，但是ai也没有好的解决方法，js不能更改浏览器的设置，这个待定            |
| **P2** | 设计缺位   | **avatar 选同一文件不触发 change** —— 换头像须切别的再切回来                                                                                                                       | 上传后清空 `input.value = ''`                                                                                                                     | 1 行                               | 零                   | 可以                                                    |
| **P3** | 设计缺位   | **无版本/快照** —— localStorage 单 key 一覆盖，误操作/清缓存 = 数据没了（`site/backup/` 目录存在但代码无写入机制）                                                                                | 加自动每 5 分钟导出到 backup 槽位 + 手动"恢复上次备份"按钮                                                                                                        | ~50 行 / 2 文件                      | 中                   | 可以                                                    |
| **P3** | 设计缺位   | **跨项目协作无运行时校验** —— README 称字段命名以 CV 为 canonical 源，但仓库无 schema.json 副本                                                                                           | 把 cv-autofill/schema/cv-superset.schema.json 软链或拷贝到 `docs/cv-schema.json`                                                                    | 1 文件                              | 零                   | 可以，我还正想做一个这个                                          |
| **P3** | 设计缺位   | **无 telemetry/error reporting** —— `console.error` 仅本地打印                                                                                                        | 加"复制错误详情"按钮 + 本地错误队列                                                                                                                         | ~30 行 / 2 文件                      | 低                   | 可以                                                    |
| **P3** | 文档化    | **多语言缺失未声明** —— UI 全硬编码中文，README 没说"仅支持中文 UI"                                                                                                                   | README 顶部加一行说明                                                                                                                               | 1 行                               | 零                   | 待批注                                                   |
...
| **P3** | 文档化    | **Markdown "标准格式"过度宣传** —— 实际只支持项目自定义 pipe 格式                                                                                                                   | README 改名"自定义 Markdown 方言"，附 sample                                                                                                          | 5 行                               | 零                   | 待批注                                                   |
...
| ------ | ------------------------------ | ----------------------- | --------------------------------------------------------------------------------------- | ------------ | ------------ | --- |
| **P1** | `experience[].achievements`    | schema 有,renderer 不渲染   | renderer 加 `<h4>工作业绩</h4><ul>` 块（已有 `lis()` helper）                                     | ~15 行 / 1 文件 | 低            | 可以  |
| **P1** | `experience[].isIntern`        | schema 有,renderer 不渲染   | renderer 加 tag 显示"实习"（类比现有 `industry/department` tag）                                   | ~5 行 / 1 文件  | 零            | 可以  |
| **P1** | `experience[].industry`        | schema 有,renderer 不渲染   | renderer tag 渲染（已有逻辑,确认无误）                                                              | ~3 行 / 1 文件  | 零            | 可以  |
| **P1** | `experience[].department`      | schema 有,renderer 不渲染   | 同上                                                                                      | ~3 行 / 1 文件  | 零            | 可以  |
| **P1** | `experience[].skillTags`       | schema 有,renderer 不渲染   | renderer 加 `<ul class="tag-list">` 块（已有逻辑）                                              | ~3 行 / 1 文件  | 零            | 可以  |
| **P2** | `profile.expectJobs` 单条目       | schema 无                | schema 加 `profile.expectJobs = [{title, jobType, salary:{low,high}, cities[]}]`，编辑器复合表单 | ~40 行 / 2 文件 | 中（schema 设计） | 可以  |
| **P2** | `profile.firstWorkDate`        | schema 无                | schema 加 date 字段，编辑器单 input                                                             | ~5 行 / 1 文件  | 低            | 可以  |
| **P2** | `education[].overseasEdu`      | schema 无                | schema 加 boolean 字段，编辑器 select 是/否                                                      | ~3 行 / 1 文件  | 低            | 可以  |
| **P2** | `language.readingWriting` 字段补漏 | schema 有但 renderer 注释漏读 | renderer 已有 `i.readingWriting` 处理，确认无误                                                  | 0 行          | 零            | 可以  |
...
- 决策 1: 那就先都要，之后再看效果决定去留
- 决策 2: 新增字段的那个表我批注了，按那个来吧
- 其它的我都在表格里批注了
- 表格里没有批注的先做成issue

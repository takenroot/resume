"""Test css mode without adding html2pdf__page-break class - just using CSS class selector"""
from playwright.sync_api import sync_playwright
import time, os

URL = 'http://localhost:8765/index.html'

def test(name, use_class, pagebreak_conf):
...
def main():
    # css mode WITHOUT adding html2pdf__page-break class
    test('css-no-class', False, "{ mode: 'css' }")
    test('css-with-class', True, "{ mode: 'css' }")
    test('css2-no-class', False, "{ mode: 'css2' }")
    test('css2-with-class', True, "{ mode: 'css2' }")
    # Also test without windowWidth constraint
    test('css-nw-no-class', False, "{ mode: 'css' }")
    test('css-nw-with-class', True, "{ mode: 'css' }")
